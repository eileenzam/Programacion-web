...
class CustomPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if request.method == 'GET':
            return request.user.has_perm(f'{model._meta.app_label}.view{model._meta.model_name}')
        if request.method == 'POST':
            return request.user.has_perm(f'{model._meta.app_label}.add{model._meta.model_name}')
        return True
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in ['PUT', 'PATCH']:
            return request.user.has_perm(f'{obj._meta.app_label}.change{obj._meta.model_name}')
        if request.method == 'DELETE':
            return request.user.has_perm(f'{obj._meta.app_label}.delete{obj._meta.model_name}')
        return True